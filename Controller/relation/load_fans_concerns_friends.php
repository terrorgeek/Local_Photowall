...
function Travers_ResultSet($sql)
{
	...
	{	
	    if($out['nick_name']!="")
		{
			...
			$result_array[$index]=array('PinYin'=>ucfirst($pinyin_value),'nick_name'=>$out["nick_name"],'login_name'=>$out["login_name"],
			                             'user_id'=>$out["user_id"],'head_photo'=>"../Project_Images/head_photo/".str_replace(" ", "%20", $out['user_id']."_".$out['head_photo']),"first_letter"=>strtoupper(substr($pinyin_value, 0,1)));
		}
		else
		{
			...
			$result_array[$index]=array('PinYin'=>ucfirst($pinyin_value),'login_name'=>$out["login_name"],'nick_name'=>$out["nick_name"],
			                             'user_id'=>$out["user_id"],'head_photo'=>"../Project_Images/head_photo/".str_replace(" ", "%20", $out['user_id']."_".$out['head_photo']),"first_letter"=>strtoupper(substr($pinyin_value, 0,1)));
		}
		$index++;		
	}
